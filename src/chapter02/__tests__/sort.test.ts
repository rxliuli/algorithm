...
describe('测试排序', () => {
  ...
  describe('归并排序', () => {
    describe('原地归并', () => {
      describe('测试 merge', () => {
        it('基本示例', () => {
          ...
          expect(merge([...arr], 0, 7, 3, Array(arr.length))).toEqual(
            sortBy(arr),
          )
        })
        it('测试合并中间的值情况', () => {
          const arr = [1, 3, 2, 4]
          expect(merge(arr, 1, 2, 1, Array(arr.length))).toEqual([1, 2, 3, 4])
        })
      })
      ...
      describe('使用插入排序对小数组排序', () => {
        ...
        it('测试排序小数组的性能', () => {
          const arr = RandomUtil.array(10000, () => RandomUtil.array(10))
          const time1 = countTime(() =>
            arr.forEach((item) => sortSmallOfInsert(item, 0, item.length - 1)),
          )
          const time2 = countTime(() =>
            arr.forEach((item) => sortOfMerge(item)),
          )
    ...
    describe('自底而上', () => {
      ...
      it('测试 sortOfMergeBU', () => {
        const arr = RandomUtil.array(100)
        expect(sortOfMergeBU([...arr])).toEqual(sortBy(arr))
      })
    })
  })
})
